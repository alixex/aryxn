...
- **Plain mode** (random key in fragment): the key is 256-bit → unbreakable. But *possession of the link = access*:
...
Why hybrid (not pure password): a pure password-derived key would let *anyone who lists the ciphertext* run an
...
P    = Argon2id(password, salt, ALG=Argon2id13, ops=MODERATE, mem=MODERATE) → 32 bytes   // crypto_pwhash
K    = BLAKE2b( R ‖ P, 32 bytes )          // crypto_generichash — secure combiner of both halves
fragment payload = "p1." + base64url(R) + "." + base64url(salt)
```
...
- **Wrong password** → wrong `P` → wrong `K` → Poly1305 authentication fails on decrypt → a single "wrong password"
  error. No decryption oracle beyond pass/fail.

Why `K = BLAKE2b(R ‖ P)`: both halves are required. Without `R` (the link) a password-holder cannot derive `K`
(`R` is 256-bit). Without the password, a link-holder must brute-force `P` (Argon2id-hardened). BLAKE2b over the
concatenation is a standard, collision-resistant combiner; `salt` already feeds `P`, so it need not be re-bound.
...
| ---- | ------- | ----------- |
| Plain (existing) | `base64url(K)` | no `p1.` prefix |
| Password (new) | `p1.` + `base64url(R)` + `.` + `base64url(salt)` | leading `p1.` |

- All base64 is **base64url without padding** (URL-safe: no `+` `/` `=` in the fragment).
- **Backward compatible**: existing shared plain links (`#/view/<chain>/<txId>/<Kb64>`) are unchanged and still
  decrypt — the parser treats any payload lacking the `p1.` prefix as a plain key. (The current `keyB64` is standard
  base64; the parser must accept both standard and url-safe base64 for the plain case to avoid breaking old links.)
- `p1` is a **version+params tag**: it pins ALG=Argon2id13 + MODERATE limits. Changing KDF params later → `p2`
  (the viewer selects params by version, so old links stay decryptable).
...
1. **`@alixex/crypto`** — add and export:
   ...
   - `toBase64Url` / `fromBase64Url` helpers (url-safe, unpadded) — or extend existing `toBase64`/`fromBase64`.
...
2. **`apps/link/src/storage.ts`**:
   ...
   - `decryptAsset(chain, txId, payload, password?)` — parse `payload`: `p1.` prefix → split `R`,`salt` → require
     `password` (throw `PASSWORD_REQUIRED` if absent) → `P = deriveArgon2idKey(pw, salt)` → `K = combineKeyHalves(R,P)`;
     else plain → `K = fromBase64(payload)`. Then decrypt as today. Wrong password → decrypt throws → map to a
...
4. **Home upload UI** (`pages/home.ts`, built in multi-account Task 9–11) — when the **Encrypt** toggle is on, reveal
   ...
   Empty → plain mode; set → password mode. Show the **security warning** (§7) whenever Encrypt is on.
...
that a *weak* password on a leaked link is brute-forceable over time.
