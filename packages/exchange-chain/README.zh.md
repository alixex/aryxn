...
`@alixex/exchange-chain` 提供了一个高级路由，能够智能地在同链 Swap（兑换）和跨链 Bridge（桥接）之间进行选择。它集成了 `@alixex/swap-multichain` 用于 DEX 操作，以及 `@alixex/cross-chain` 用于桥接服务（通过 LiFi）。
...
- **历史记录追踪**：通过 `@alixex/query-chain` 集成交易历史记录。
