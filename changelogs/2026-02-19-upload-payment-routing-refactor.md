...
- 账户管理删除任意账户均加入高风险提示与倒计时确认：删除按钮需等待倒计时结束后才可点击；Arweave 账户额外提示与数据管理相关的不可逆影响，避免误删导致不可恢复的数据访问问题。
...
6. 账户管理删除任意账户需有倒计时；删除 AR 账户需额外提示数据管理相关风险。
...
7. 在账户管理尝试删除任意账户：应先出现统一高风险提示与倒计时，倒计时结束后才可删除。
8. 删除 Arweave 账户时应额外出现“数据管理相关风险”提示。
...
- [ ] 账户管理删除任意账户时有倒计时与统一风险提示
- [ ] 账户管理删除 Arweave 账户时有额外的数据管理风险提示
